...
CREATE POLICY "own_fixed_bills" ON fixed_bills FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "own_installments" ON installments FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "own_monthly_summary" ON monthly_summary FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "own_savings" ON savings FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
